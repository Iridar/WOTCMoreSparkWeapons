class X2Item_HeavyAutogun_MK2 extends X2Item config(AutogunHeavyWeapon);
...
static function X2DataTemplate Create_Item()
{
	...
	`CREATE_X2TEMPLATE(class'X2WeaponTemplate', Template, 'IRI_Heavy_Autogun_MK2');
	...
	Template.GameArchetype = "IRISparkHeavyWeapons.Archetypes.WP_Heavy_LAC_MK2";

	//	Regular archetype for soldiers using powered armor. Both of these also double for SPARKs without BIT.
	Template.AltGameArchetype = "IRISparkHeavyWeapons.Archetypes.WP_Heavy_LAC_Powered_MK2";
	Template.ArmorTechCatForAltArchetype = 'powered';

	GameArch.UseGameArchetypeFn = class'X2Item_HeavyAutogun'.static.SparkHeavyWeaponCheck;
	GameArch.ArchetypeString = "IRISparkHeavyWeapons.Archetypes.WP_Heavy_LAC_Spark_with_BIT_MK2";
	Template.AltGameArchetypeArray.AddItem(GameArch);

	GameArch.UseGameArchetypeFn = class'X2Item_HeavyAutogun'.static.BitHeavyWeaponCheck;
	GameArch.ArchetypeString = "IRISparkHeavyWeapons.Archetypes.WP_Heavy_LAC_Bit_MK2";
	Template.AltGameArchetypeArray.AddItem(GameArch);
